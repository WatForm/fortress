...
object ScopeSubtype {
    ...
    def subtypePred(sort: Sort): String = s"__@${sort}"
    ...
    def addBoundsPredicates(term: Term): Term = term match {
        ...
        case Iff(l, r) => Implication(addBoundsPredicates(l), addBoundsPredicates(r))
        ...
        case Exists(vars, body) => {
            val predApps = vars.map(av => App(subtypePred(av.sort), av.variable))
            Exists(vars, And.smart(predApps :+ body))
        }
        case Forall(vars, body) => {
            val predApps = vars.map(av => App(subtypePred(av.sort), av.variable))
            Forall(vars, Implication(And.smart(predApps), body))
        }
    }
}