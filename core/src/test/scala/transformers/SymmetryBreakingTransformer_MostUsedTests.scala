...
class SymmetryBreakingTransformer_MostUsedTests extends UnitSuite {
    ...
    test("constants") {
        ...
        transformer(ProblemState(theory, scopes)) should be(ProblemState(expected, scopes, Set.empty, Set.empty, expectedRangeFormulas, List.empty, Flags(distinctConstants = true, haveRunNNF = false, verbose = false)))
    }
