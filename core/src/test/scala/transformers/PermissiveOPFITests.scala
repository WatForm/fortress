...
import fortress.transformers.Integers.PermissiveOPFITransformer


class PermissiveOPFITests extends UnitSuite {
    val transformer = PermissiveOPFITransformer
    val intSize = 8
    val min = -4
    val max = 3
    ...
    test("basic literals") {
        ...
        transformedEq should be ( // 1 < 2 & !(1 overflows || 2 does overflows)
            AndList(Seq(
                ...
                Not(OrList(Seq( 
                    App(outOfBoundsName, Seq(IntegerLiteral(1))),
                    App(outOfBoundsName, Seq(IntegerLiteral(2)))
                )))
            ))
        )
        val resultSig = result.theory.signature
        resultSig.functionDefinitions should have size (3)  // To
    ...
    test("large literals") {
        ...
        transformedGE should be (
            And(
                Not(BuiltinApp(IntGE, Seq(IntegerLiteral(1000), IntegerLiteral(2)))),
                Not(Or(
                    App(outOfBoundsName, Seq(IntegerLiteral(1000))),
                    App(outOfBoundsName, Seq(IntegerLiteral(2)))
                ))
            )
        )
    ...
    test("variables") {
        ...
        resultAxiom should be (
            AndList(Seq(
                ...
                Not(App(outOfBoundsName, Seq(BuiltinApp(IntPlus, Seq(App(toInt, Seq(Var("x"))), App(toInt, Seq(Var("x"))))))))
            ))
        )
    }
    ...
    test("integer predicates") {
        ...
        resultAxiom2 should be (
            AndList(Seq( // f(fromInt(toInt([x])+1)) && Inbounds(toInt(x)+1)
                ...
                // In range
                Not(App(outOfBoundsName, Seq( // TODO arg to isInBound is being wrapped in fromInt, which is wrong!!!
                    BuiltinApp(IntPlus, Seq( // toInt(x) + 1
