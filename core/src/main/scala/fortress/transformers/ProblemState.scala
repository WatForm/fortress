...
case class ProblemState private(
    ...
    Errors.Internal.precondition(scopes.values.forall(_._1 > 0), "Scopes must be positive")
    ...
    def withScopes(newscopes: Map[Sort, (Int, Boolean)]): ProblemState = {
        new ProblemState(            
            theory,    
...
object ProblemState {
    ...
    def apply(theory: Theory, scopes: Map[Sort, (Int, Boolean)]): ProblemState = {
        // Compute the scopes for enum sorts
        val enumScopes = theory.signature.enumConstants.map {
            case (sort, enumValues) => sort -> (enumValues.size, true)
        }.toMap
