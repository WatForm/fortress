...
trait OPFITransformer extends ProblemStateTransformer {
    ...
    // Permissive and strict differ in how they handle quantifiers, so we make them extend this
    ...
    def apply(problemState: ProblemState): ProblemState = {
        ...
        */
        ...
        def transform(term: Term, down: DownInfo): (Term, Set[Term]) = term match {
            ...
            case Forall(quants, body) => handleForall(quants, body, down, transform, opfiSort)

            case Exists(quants, body) => handleExists(quants, body, down, transform, opfiSort)
