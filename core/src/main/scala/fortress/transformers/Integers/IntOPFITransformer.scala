    ...
    def apply(problemState: ProblemState): ProblemState = {
        ...
        def transform(term: Term, down: DownInfo = blankDown): (Term, UpInfo) = term match {
            ...
            case IfThenElse(condition, ifTrue, ifFalse) => {
                ...
                // we check the type of the transformed value so that it typechecks correctly
        ...
        */
        ...
        def unapplyInterp(interp: Interpretation): Interpretation = {
            ...
            interp.withoutFunctionDefinitions(Set(castToIntDefn, castFromIntDefn, isInBounds)).applySortSubstitution(substitute)
