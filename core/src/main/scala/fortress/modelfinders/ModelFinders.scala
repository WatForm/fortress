...
/**
  ...
  * Extends AutoCloseable because it may hold a resource, such as an open connection to an SMT solver, which must be closed.
  ...
  passed three important parameters(theory,compiler, solver), which must
  be set by user; other settings can have defaults (timeouts) or be empty (scopes)

  TODO: make a trait (.h for this file)

  Note: this class can't have arguments if we want to use a straightforward string
  registry to look it up!
  */
class ModelFinder() extends AutoCloseable {
