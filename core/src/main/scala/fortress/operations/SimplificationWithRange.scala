...
class SimplifierWithRange(rangeRestrictions: Set[RangeRestriction]) {
    def simplify(term: Term): Term = {
        def simplifyFull(term: Term): Term = term match {
            ...
            case Eq(_, _) | App(_, _) | BuiltinApp(_, _) | Closure(_, _, _)
                | ReflexiveClosure(_, _, _) => simplifyStep(term)
    ...
    def simplifyStep(term: Term): Term = term match {
        ...
        //     }
        // }
        case Eq(t, d @ DomainElement(_, _)) if rangeRestrictions exists (rr => rr.term == t) => {
            val rrs = rangeRestrictions filter (rr => rr.term == t)
            for(rr <- rrs) {
                if(!rr.values.contains(d)) {
                    // println("Could simplify!")
                    return Bottom
                }
            }
            term
        }
        case Eq(d @ DomainElement(_, _), t) if rangeRestrictions exists (rr => rr.term == t) => {
            val rrs = rangeRestrictions filter (rr => rr.term == t)
            for(rr <- rrs) {
                if(!rr.values.contains(d)) {
                    // println("Could simplify!")
