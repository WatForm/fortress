...
object Symmetry {
    ...
    def rainbowFunctionLT(
        ...
        Errors.Internal.precondition(f.argSorts.forall(state.scope(_)._1 >= 2))
        Errors.Internal.precondition(state.scope(f.resultSort)._1 >= 2)
        
        val (ltDecl, ltDefn) = sortLtDefinition(f.resultSort, state.scope(f.resultSort)._1)
        ...
        val (constraints, rangeRestrictions): (Seq[Term], Seq[RangeRestriction]) = f match {
            ...
            case FuncDecl(fname, Seq(argSort), resultSort) => {
                ...
                val ltConstraints: Seq[Term] = for(i <- 1 until state.scope(argSort)._1) yield {
                    App(LT,
            ...
            case FuncDecl(fname, Seq(argSort1, argSort2), resultSort) => {
                ...
                val ltConstraintsArg1: Seq[Term] = for(i <- 1 until state.scope(argSort1)._1) yield {
                    App(LT,
                ...
                val r1 = scala.math.min(state.scope(argSort1)._1, state.scope(resultSort)._1)
                ...
                val ltConstraintsArg2: Seq[Term] = for(i <- 2 until state.scope(argSort2)._1) yield {
                    App(LT,
