...
class LowArityFirstMostUsedFunctionFirstOrder(stalenessTrackerParam: StalenessTracker, remainingTrackerParam: RemainingIdentifiersTracker)
  extends SelectionHeuristicWithConstants(stalenessTrackerParam: StalenessTracker, remainingTrackerParam: RemainingIdentifiersTracker) {
    override def name = "Low Arity First, Function First, Then Most Used"
    ...
    def preprocess(profilingInfo: Map[Declaration, Int]): Unit = {
        ...
        val functionDeclarationList: Seq[FuncDecl] = remainingTracker.remainingFuncDecls.toSeq sortWith fnLessThan
        preplannedOrder = (remainingTracker.remainingConstants.toSeq sortWith consLessThan) ++
          functionDeclarationList.filter(acceptableFunction) ++ functionDeclarationList.filter(acceptablePredicate)
    }
...
object LowArityFirstMostUsedFunctionFirstOrderFactory extends SelectionHeuristicWithConstantsFactory {
    override def name = "Low Arity First, Function First, Then Most Used"

    def create(stalenessTracker: StalenessTracker, remainingTracker: RemainingIdentifiersTracker): SelectionHeuristicWithConstants = new LowArityFirstMostUsedFunctionFirstOrder(stalenessTracker, remainingTracker)
}