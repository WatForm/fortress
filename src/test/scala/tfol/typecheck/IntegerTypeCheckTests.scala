...
class IntegerTypeCheckTests extends FunSuite with Matchers {
    ...
    test("basic integer literal") {
        ...
        IntegerLiteral(5).typeCheck(sig).sort should be (IntType)
    }
    
    test("basic integer literal, integers not in signature") {
        val sig = Signature.empty
        a [fortress.data.TypeCheckException.UnknownType] should be thrownBy (IntegerLiteral(5).typeCheck(sig))
    }
    
    test("basic +, -, *") {
        val sig = Signature.empty.withIntegers
        App(plus, IntegerLiteral(5), IntegerLiteral(6)).typeCheck(sig).sort should be (IntType)
    }
