...
*/
class SymmetryBreakingTransformer_Imp1(selectionHeuristic: SelectionHeuristic) extends ProblemStateTransformer {
    ...
    def apply(problemState: ProblemState): ProblemState = problemState match {
        case ProblemState(theory, scopes, skc, skf, rangeRestricts, unapplyInterp) => {
            val breaker = new Imp1SymmetryBreaker(theory, scopes)
