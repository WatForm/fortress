...
*/
class SymmetryBreakingTransformerTWO_BETA(selectionHeuristic: SelectionHeuristic) extends ProblemStateTransformer {
    ...
    def apply(problemState: ProblemState): ProblemState = problemState match {
        case ProblemState(theory, scopes, skc, skf, rangeRestricts, unapplyInterp) => {
            ...
            println(breaker.newRangeRestrictions.toList.mkString("\n"))
