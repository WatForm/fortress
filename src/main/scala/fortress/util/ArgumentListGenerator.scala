...
class ArgumentListGenerator(scopes: PartialFunction[Sort, Int]) {
    ...
    def allArgumentListsOfFunction(f: FuncDecl): Iterable[Seq[DomainElement]] = {
        val possibleRangeValues = DomainElement.range(1 to scopes(f.resultSort), f.resultSort)
        ...
        val seqOfDomainSeqs: IndexedSeq[IndexedSeq[DomainElement]] = f.argSorts.toIndexedSeq.map (sort => {
            val Di: IndexedSeq[DomainElement] = DomainElement.range(1 to scopes(sort), sort)
            Di
        })
