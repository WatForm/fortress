...
class Skolemizer(topLevelTerm: Term, signature: Signature, nameGen: NameGenerator) {
    ...
    */
    ...
    class SkolemVisitor extends TermVisitorWithTypeContext[Term](signature) {
        ...
        override def visitClosure(term: Closure): Term = term.mapArguments(visit)

        override def visitReflexiveClosure(term: ReflexiveClosure): Term = term.mapArguments(visit)
        ...
        override def visitExistsInner(term: Exists): Term = {
            ...
            for(av <- term.vars) {
                ...
                if(freeVars.size == 0) {
                    ...
                    for(v <- freeVars) {
                        val sortMaybe: java.util.Optional[Sort] = lookupSort(v)
                        Errors.assertion(sortMaybe.isPresent, "Sort of variable " + v.name + " could not be found")
