...
object Substituter {
    def apply(x: Var, s: Term, t: Term, nameGen: NameGenerator): Term = {
        ...
        def sub(t: Term): Term = t match {
            ...
            case Closure(f, args, arg1, arg2) => Closure(f, args.map(sub), sub(arg1), sub(arg2))
            case ReflexiveClosure(f, args, arg1, arg2) => ReflexiveClosure(f, args.map(sub), sub(arg1), sub(arg2))
        ...
        def avoidCapture(vars: Seq[AnnotatedVar], body: Term) = {
            ...
            if (substitutions.size > 0) {
                val newBody = body.recklessSubstitute(substitutions.toMap)
