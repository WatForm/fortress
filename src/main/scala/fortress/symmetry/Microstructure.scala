    ...
    val constraintEdges: Map[Term, Set[HyperEdge]] = {
        ...
        for(axiom <- theory.axioms) {
            ...
            val constSymbolsOfAxiom: Set[AnnotatedVar] =  {
                val constantsIn = fortress.msfol.operations.SymbolAccumulator.constantsIn(axiom)
                constantsIn.map(x => theory.signature.queryConstant(Var(x)).get)
            }
            val funcSymbolsOfAxiom: Set[FuncDecl] = {
                val functionsIn = fortress.msfol.operations.SymbolAccumulator.functionsIn(axiom)
